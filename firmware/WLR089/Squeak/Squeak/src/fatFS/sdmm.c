...
extern struct spi_module spi_instance_app;
#define CS_PIN		PIN_PA22
...
#define	CS_H()		port_pin_set_output_level(CS_PIN, 1)
#define CS_L()		port_pin_set_output_level(CS_PIN, 0)
...
)
{
	...
	do {
		...
		spi_transceive_wait(&spi_instance_app, d, &dummy);
	} while (--bc);
}
...
)
{
	...
	do {
		spi_transceive_wait(&spi_instance_app, 0xFF, &readVal);
		*buff++ = (uint8_t)readVal;
	} while (--bc);
}
...
/*-----------------------------------------------------------------------*/
/* mmc_deselect the card and release SPI bus                                 */
/*-----------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------*/
/* mmc_select the card and wait for ready                                    */
/*-----------------------------------------------------------------------*/

static
int mmc_select (void)	/* 1:OK, 0:Timeout */
{
	BYTE d;
...
)
{
	...
	/* mmc_select the card and wait for ready except to stop multiple block read */
	if (cmd != CMD12) {
		mmc_deselect();
...
)
{
	...
	port_pin_set_config(CS_PIN, &pin_conf);
	port_pin_set_output_level(CS_PIN, 1);
